...
@auth_router.post(
    "/send-code",
    response_model=TwilioVerificationModel,
    status_code=status.HTTP_201_CREATED,
)
async def send_otp_code_route(
    phonenumber: UserPhonenumber, twilio_service=Depends(get_twilio_service)
):
...
async def verify_otp_code_route(
    verification_code: VerifyOTPModel, twilio_service=Depends(get_twilio_service)
):
...
async def signup_user_route(
    ...
    try:
        ...
        valid_token = verify_phone_verification_token(
            token=token_obj, phonenumber=user_phonenumber
        )
        ...
        await create_connection(
            ...
            user2=created_user_phonenumber,
            session=session,
        )
