...
def send_OTP_text(
    phonenumber: UserPhonenumber, service: Client
) -> TwilioVerificationModel:
    """
    ...
    try:
        verification = service.verifications.create(
            to=str(phonenumber.phonenumber), channel="sms"
        )
...
def verify_OTP_text(
    verification_code: VerifyOTPModel, service: Client
) -> TwilioVerificationModel:
    """
    ...
    verification_check = service.verification_checks.create(  # Removed await, as this is a synchronous call
        to=str(verification_code.phonenumber), code=verification_code.code
    )
    return TwilioVerificationModel(
